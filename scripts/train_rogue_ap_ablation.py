#!/usr/bin/env python3
"""Run baseline-current vs strict-feature ablation for Rogue AP detection."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("results/rogue_ap_ablation_timing_dropped"))
...
def strict_drop_columns(columns: list[str]) -> list[str]:
    out = []
    for col in columns:
        low = col.lower()
        if any(pattern in low for pattern in STRICT_DROP_PATTERNS):
            out.append(col)
    return out
...
def main() -> int:
    ...
    strict_drops = strict_drop_columns(feature_cols_all)
    variants = {
        "baseline_current": feature_cols_all,
        "strict_ablation": [c for c in feature_cols_all if c not in strict_drops],
    }
    (args.output_dir / "ablation_drop_columns.json").write_text(
        json.dumps({"strict_drop_columns": strict_drops, "has_source_file": has_source_file}, indent=2),
        encoding="utf-8",
    )
    ...
    summary = ["Rogue AP strict ablation summary", ""]
    ...
    summary.extend(["", "Strict dropped features:"])
    summary.extend(f"- {c}" for c in strict_drops)
